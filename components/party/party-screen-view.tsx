...
export function PartyScreenView({
            ...
            <div className="mx-auto grid w-full max-w-sm gap-3 text-center">
              <div className="grid aspect-square place-items-center rounded-[1.5rem] border-4 border-dashed border-party-blue/45 bg-surface-sky/70 p-4 shadow-outline">
                {joinUrl && qrDataUrl ? (
                  ...
                  <img src={qrDataUrl} alt={copy.qrAlt} className="h-full w-full rounded-[1rem]" />
                ) : (
                  <div className="space-y-3">
                    <QrCode className="mx-auto h-20 w-20 text-party-blue-deep" aria-hidden="true" />
                    <p className="font-display text-3xl font-extrabold">{copy.qrPlaceholder}</p>
                  </div>
                )}
              </div>
              <p className="break-all text-lg font-extrabold">{joinUrl ?? copy.qrPlaceholder}</p>
            </div>
          </div>
        </PaperPanel>
      </section>
  ...
  if (projection.phase === "answer_reveal" || projection.phase === "waiting_for_host") {
    const correctDistribution = projection.answerDistribution.find((row) => row.isCorrect);
          ...
          <div className="space-y-4 lg:space-y-5">
            ...
            <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
              <div className="relative overflow-hidden rounded-[1.5rem] border-4 border-party-green bg-[#e7f5df] p-5 shadow-[0_12px_0_hsl(var(--party-green)/0.16)] lg:p-7">
                <Sparkles className="absolute right-5 top-5 h-10 w-10 text-party-yellow" aria-hidden="true" />
            ...
            <div className="grid gap-2 sm:grid-cols-3">
              {projection.answerDistribution.map((row) => (
                ...
                >
                  ...
                  <span className="font-display text-3xl text-foreground">{row.count}</span>
                </div>
              ))}
            </div>

            <p className="text-center text-sm font-extrabold text-muted-foreground">
              {copy.submitted}: {projection.submittedCount} · {copy.timedOut}: {projection.timedOutCount} · {copy.nextUp}
            </p>
          </div>
        </PaperPanel>
      </section>
  ...
  if (projection.phase === "leaderboard") {
    return (
      <section className="mx-auto grid min-h-[calc(100vh-4rem)] w-full max-w-7xl content-center gap-4">
        {header}
        <LeaderboardPanel copy={copy} projection={projection} final={false} />
      </section>
    );
  }
  ...
  if (projection.phase === "finished") {
    ...
    return (
      <section className="mx-auto grid min-h-[calc(100vh-4rem)] w-full max-w-7xl content-center gap-4">
        {header}
        <PaperPanel tone="celebration" className="relative overflow-hidden p-6 text-center lg:p-10">
          <div className="pointer-events-none gingham-yellow absolute inset-0 opacity-35" aria-hidden="true" />
            ...
            {winner ? (
              <>
                ...
                <p className="font-display text-4xl font-extrabold text-party-blue-deep lg:text-5xl">
                  {winner.score}/{projection.totalQuestions}
                </p>
              ...
              <h1 className="font-display text-6xl font-extrabold">{copy.finished}</h1>
            )}
            <div className="mx-auto grid max-w-3xl gap-2 pt-2 sm:grid-cols-3">
              {projection.leaderboard.slice(0, 3).map((row) => (
                <div key={row.guestId} className="flex items-center gap-3 rounded-[1rem] border border-border bg-surface-paper/90 p-3 text-left shadow-lift">
                  <span className="font-display text-2xl font-extrabold text-party-orange">#{row.rank}</span>
                  <ParticipantAvatar avatar={row.avatar} displayName={row.displayName} size="md" />
                  <span className="min-w-0 flex-1 truncate font-extrabold">{row.displayName}</span>
                  <span className="font-display text-2xl font-extrabold text-party-blue-deep">{row.score}</span>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <section className="mx-auto grid min-h-[calc(100vh-4rem)] w-full max-w-7xl content-center gap-4">
      ...
      <PaperPanel tone="display" className="p-6 lg:p-9">
        <div className={`grid gap-6 ${projection.phase === "question_active" ? "lg:grid-cols-[1fr_18rem]" : ""}`}>
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <BirthdayBadge tone="yellow">{projection.questionNumber}/{projection.totalQuestions}</BirthdayBadge>
            </div>
            ...
            {question && (projection.phase === "question_active" || projection.phase === "question_locked") ? (
              <div className="grid gap-3 sm:grid-cols-3">
                {question.options.map((option) => (
                  <div key={option.id} className="rounded-[1.1rem] border border-border bg-surface-paper px-4 py-4 text-xl font-extrabold shadow-lift">
                    {option.label[locale]}
                  </div>
                ))}
              </div>
            ...
            {projection.phase === "question_locked" ? (
              <div className="rounded-[1.1rem] border border-party-orange/30 bg-surface-highlight p-4 text-xl font-extrabold">
                {copy.waitingReveal} · {copy.submitted}: {projection.submittedCount} · {copy.timedOut}: {projection.timedOutCount}
              </div>
            ) : null}
          </div>
          {projection.phase === "question_active" ? (
            <aside className="grid content-start gap-4">
              ...
              <BirthdayBadge tone="blue" className="justify-center py-3 text-base">
                {copy.submitted}: {projection.submittedCount}/{projection.participantCount}
              </BirthdayBadge>
            </aside>
          ) : null}
        </div>
    ...
    <PaperPanel tone="celebration" className="p-6 lg:p-9">
      <div className="mb-5 flex items-center gap-3">
        ...
        <h1 className="font-display text-5xl font-extrabold text-foreground">
          {final ? copy.finalResults : copy.leaderboard}
        </h1>
      </div>
      <div className="grid gap-3">
        {projection.leaderboard.map((row) => (
          <div key={row.guestId} className="grid grid-cols-[4rem_auto_1fr_5rem] items-center gap-4 rounded-[1.1rem] border border-border bg-surface-paper px-5 py-4 text-3xl font-extrabold shadow-lift">
