...
export async function GET() {
  if (!isSupabaseConfigured()) {
    return NextResponse.json(
      { error: { code: "supabase_not_configured", message: "Certificate service is not configured." } },
      { status: 503 }
    );
  }
  ...
  try {
    const result = await loadWinnerCertificateContext(participantSession);
    ...
    return NextResponse.json(
      { error: { code: "temporary_server_failure", message: "The certificate could not be prepared yet." } },
      {
        status: 500,
        headers: { "Cache-Control": "private, no-store" }
      }
