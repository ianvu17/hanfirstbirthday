...
test("final leaderboard advances to finished without overflowing the question index", () => {
  ...
  const finished = mustAccept(state, {
    type: "ADVANCE_FROM_LEADERBOARD",
    now: 5000,
  });
  ...
  assert.equal(
    selectHostCapabilities(finished).canAdvanceFromLeaderboard,
    false,
  );
});
...
test("invalid transitions are rejected with structured errors", () => {
  ...
  const reveal = processPartyCommand(
    state,
    { type: "REVEAL_ANSWER", now: 0 },
    config,
  );
  ...
  assert.equal(
    reveal.ok === false && reveal.error.code,
    "invalid_phase_transition",
  );
  assert.equal(
    leaderboard.ok === false && leaderboard.error.code,
    "invalid_phase_transition",
  );
});
...
test("submission locks once, exact retry is idempotent, conflicting retry is rejected", () => {
  ...
  const acceptedPoints =
    accepted.state.responses[`test-guest-01::${questionId}`].pointsAwarded;
  ...
  assert.equal(
    exactRetry.state.responses[`test-guest-01::${questionId}`].pointsAwarded,
    acceptedPoints,
  );
  ...
  assert.equal(
    conflict.ok === false && conflict.error.code,
...
test("idempotent retries clear stale local errors without changing revision", () => {
  ...
  state = mustAccept(duplicateRegister.state, {
...
test("timeout materializes unanswered guest responses and scoring uses persisted time points", () => {
  let state = createInitialPartyState(config);
  state = mustAccept(state, { type: "PREPARE_FIRST_QUESTION", now: 0 });
  state = mustAccept(state, { type: "REVEAL_CHOICES", now: 100 });
  state = mustAccept(state, {
  ...
  assert.equal(winner?.score, 1998);
  assert.equal(winner?.pointsGained, 1998);
...
test("projections hide correct answer until reveal", () => {
  ...
  assert.equal(
    "correctOptionId" in (activeProjection.currentQuestion ?? {}),
    false,
  );
  assert.equal("correctOptionId" in (activeGuest.currentQuestion ?? {}), false);
  assert.equal(activeGuest.reveal, null);
  assert.ok(
    activeProjection.answerDistribution.every((row) => row.isCorrect === null),
  );
  ...
  assert.equal(
    revealProjection.correctOption?.id,
...
test("manual host lock is not part of the production flow", () => {
  ...
  assert.equal(
    manualLock.ok === false && manualLock.error.code,
    "command_not_allowed",
  );
});
...
test("correct answer cannot be revealed until the deadline closes answering", () => {
  ...
  assert.equal(
    earlyReveal.ok === false && earlyReveal.error.code,
    "invalid_phase_transition",
  );
