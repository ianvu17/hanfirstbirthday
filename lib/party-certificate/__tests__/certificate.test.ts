...
test("certificate filename is safe and stable", () => {
  assert.equal(certificateFileName("  Nguyễn / Ian?!  "), "han-mastermind-nguyen-ian.pdf");
...
test("English, Vietnamese, photo, preset, fallback, and long-name certificates are one A4 landscape page", async () => {
  ...
  const cases = [
    ...
    context({ displayName: "Alexandra Nguyễn-Vũ With An Exceptionally Long Celebration Name" }),
    context({
...
test("certificate endpoint derives identity server-side and sets private download headers", () => {
  ...
  assert.equal(repository.includes('sessionResult.data.phase !== "finished"'), true);
